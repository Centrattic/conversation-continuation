...
from src.config import MODEL_NAME, RESULTS_FOLDER, bnb_config, DATA_PATH, OLD_RESULTS_FOLDER
...
# initialize new embeddings to be similar to existing ones
embeddings = model.get_input_embeddings()

with torch.no_grad():
    # Example: different reference tokens for each special token
    special_token_map = {
        special_tokens[0]: ["ri", "ya", "_R"],
        special_tokens[1]: ["_Owen"]
    }
    for special_token, ref_tokens in special_token_map.items():
        tok_id = tokenizer.convert_tokens_to_ids(special_token)
        ref_ids = tokenizer.convert_tokens_to_ids(ref_tokens)
        ref_ids = [i for i in ref_ids if i != tokenizer.unk_token_id]

        if not ref_ids:
            print(f"No valid reference tokens for {special_token}")
            continue

        mean_vec = embeddings.weight[ref_ids].mean(dim=0)
        embeddings.weight[tok_id] = mean_vec

# can't train only some embeddings, must train the full embedding layer
...
def tokenize(example):
    ...
    tokenized = tokenizer(
        full_text, 
        truncation=True, # if number toks exceeds model's max len, 2048 for mistral (challenge with longer context for msgs)
        padding=False
    )
    # tokenized["labels"] = tokenized["input_ids"].copy() doing before padding, which is cooking tokenizer when pad in data collator
    return tokenized
...
training_args = TrainingArguments(
    ...
    learning_rate=1e-4,  # higher learning rate vs. lora, but grads exploding, so lowering lr
    fp16=False,
    logging_steps=50,
    save_strategy="steps",
    save_steps=1000,
...
trainer = Trainer(
    ...
    callbacks=[
        # welp i no longer have test.json, I guess we see how memorization works now, tbh very curious on this
        SampleGenerationCallback(tokenizer, log_path = output_dir / "mid_completions.json", test_data_path = Path(DATA_PATH) / "train.json", every_n_steps=500),
