...
while (1):
    ...
    if enable_tda:
        lora_out, acts = generate_with_activations(
            lora_model,
            "".join(history),
            tokenizer,
        ...
        lora_out = generate_with_steering(lora_model,
                                          "".join(history),
                                          tokenizer,
                                          steering_vector,
                                          max_new_tokens=max_new_tokens,
        ...
        lora_out, perplexity = generate_with_ppl(lora_model,
                                                 "".join(history),
                                                 tokenizer,
                                                 max_new_tokens=max_new_tokens)
    ...
    if enable_tda:
        sel_acts = acts[
            1:index +
            1]  # a list, so grabbing the ones that are from (1, prompt_len+1, H) to (1, prompt_len+index, H)
        print("TESTING", index, len(sel_acts))
        gen_acts = torch.stack([a[0, -1, :] for a in sel_acts],
                               dim=0)  # (num_gen_tokens, hidden_size)
        ...
        top_train_samples, _ = find_topk_train_samples(
            mistral_cache,
            agg_gen_acts,
            k=1,
            author_id=str(FRIEND_ID),
            agg_method=agg)
