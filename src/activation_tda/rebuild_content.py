...
def rebuild_content_map(
    ...
    # 3) write out
    output_path.write_text(json.dumps(content_map, ensure_ascii=False, indent=2))
...
if __name__ == "__main__":
    rebuild_content_map(
        ...
        author_index_path=Path(f"{RESULTS_FOLDER}/activation_cache/final.author_index.json"),
        output_path=Path(f"{RESULTS_FOLDER}/activation_cache/final.content_map.json"),
    )
