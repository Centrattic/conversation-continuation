...
def load_constitutions(base_path: Path) -> Dict[str, str]:
    ...
    try:
        with open(base_path / "riya_constitution.txt", "r",
                  encoding="utf-8") as f:
            riya_constitution = f.read()
        with open(base_path / "friend_constitution.txt", "r",
                  encoding="utf-8") as f:
            friend_constitution = f.read()
        return {RIYA_NAME: riya_constitution, FRIEND_NAME: friend_constitution}
    except FileNotFoundError as e:
        print(
            f"Error: Constitution file not found. Make sure they are in {base_path}"
        )
        raise e
...
def get_speaker_from_prompt(prompt: str) -> str:
    ...
    if f"[{RIYA_NAME}]" in last_line:
        ...
        raise ValueError(
            f"Cannot determine speaker from prompt's last line: {last_line}")


def build_prompt(
        conversation_prompt: str,
        original_completion:
    str,  # this will simply be a single output, as in train.json
        persona_constitution: str,
        persona_name:
    str,  # "Riya" or "Friend" to best reference the conversation  
):
...
def get_revised_completion(
    ...
    system_prompt, user_message = build_prompt(conversation_prompt,
                                               original_completion,
                                               persona_constitution,
                                               persona_name)
