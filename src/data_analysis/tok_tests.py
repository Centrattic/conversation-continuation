...
for i in range(len(data)):
    ...
    tok_out = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=1024)

    tot += len(tok_out)

print(f"Total text token count: {tot}")

test_tokenizer_vals = ["rtyagi86", "kidnamedowen", "riya", "Owen", "Riya", "owen", '[Riya]', '[Owen]', '[Riya]:', '[Owen]:', 'Riya:', 'Owen:']
...
for i in test_tokenizer_vals:
    tok_out = tokenizer(i, return_tensors="pt", truncation=True, max_length=1024)
    # ids = tokenizer.convert_tokens_to_ids(tok_out[0])

    ids = tok_out["input_ids"][0].tolist() # tensor to list
    attn_mask = tok_out["attention_mask"] # without [0] we have this 2D thing, handling batch size > 1 I think
