...
while(1):
    ...
    index_friend = friend_out.find(f"[{RIYA_NAME}]") # oh but sometimes outputs [R token and not [RIYA] both tokens (or more than 1?), since that is the split
    if index_friend == -1:
    ...
    index_riya = riya_out.find(f"[{FRIEND_NAME[0]}") # assuming friend token split [first letter for now, should have more nuance later
