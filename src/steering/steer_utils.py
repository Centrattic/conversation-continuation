...
@torch.no_grad()
def generate_steering_vector(model,
                             tokenizer,
                             steer_dict: dict,
                             pos_alpha: float = 1.0,
                             neg_alpha: float = 1.0,
    ...
    for text, weight in steer_dict.items():
        ...
        prompt_hidden = prompt_out.hidden_states[layer_from_last].to(
            device).to(model_dtype)  # [1, prompt_len, H]
    ...
    avg_pos_vector = torch.stack(pos_vectors).mean(dim=0).to(
        model_dtype)  # [num_prompts, H] -> [H]
    avg_neg_vector = torch.stack(neg_vectors).mean(dim=0).to(
        model_dtype)  # [num_prompts, H] -> [H]
