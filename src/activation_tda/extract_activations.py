...
from src.activation_tda.tda_utils import FinalLayerActivationCache, make_hash, pad_or_truncate
...
def extract_final_hidden(
    ...
    final_hidden = out.hidden_states[-1]  # (B, S, H)
    return final_hidden

def populate_cache_from_csv(
    csv_path: Path,
    cache: FinalLayerActivationCache,
    model: PreTrainedModel,
    tokenizer: PreTrainedTokenizerBase,
    device: str,
    max_seq_len: int,
    ...
    for i in tqdm(range(0, len(rows_to_add), batch_size), desc="Populate cache"):
        ...
        acts = extract_final_hidden(model, tokenizer, texts, device, max_seq_len)  # (B, S, H)
...
cache = FinalLayerActivationCache(hidden_size, max_seq_len=seq_len)

print("Populating cache")
populate_cache_from_csv(Path(DATA_PATH), cache, lora_model, tokenizer, device="cuda", max_seq_len=seq_len)
